...
export async function createServer (dynamodb?: DynamoDB, docClient?: DynamoDB.DocumentClient): Promise<Express.Application> {
  ...
  return createExpressApp(dynamodb, docClient)
}

function createExpressApp(dynamodb: DynamoDB, docClient: DynamoDB.DocumentClient): Express.Application {
  const app = express()
  app.set('json spaces', 2)
  app.set('view engine', 'ejs')
  app.set('views', path.resolve(__dirname, '..', 'views'))

  const listTables = (params: Parameters<DynamoDB['listTables']>[0]) => dynamodb.listTables(params).promise()
  const describeTable = (...args: Parameters<DynamoDB['describeTable']>) => dynamodb.describeTable(...args).promise()
  const getItem = (...args: Parameters<DynamoDB.DocumentClient['get']>) => docClient.get(...args).promise()
  const putItem = (...args: Parameters<DynamoDB.DocumentClient['put']>) => docClient.put(...args).promise()
  const deleteItem = (...args: Parameters<DynamoDB.DocumentClient['delete']>) => docClient.delete(...args).promise()
  ...
  const listAllTables = (lastEvaluatedTableName: string | null, tableNames: string[]): Promise<(DynamoDB.TableDescription | undefined)[]> => {
    return listTables({ ExclusiveStartTableName: lastEvaluatedTableName })
      .then(data => {
        ...
        return Promise.all(
          tableNames.map(TableName => describeTable({ TableName }).then(data => data.Table))
        )
      })
  }
